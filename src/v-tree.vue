<template>
	<div class='cus_vtree_wrap' @click.capture='clickNodeWrap'>
		<v-tree-item :treeData='treeData' :clickFun='clickFun' :checkBox='checkBox' :class='{tree_root_lonely:treeData.length === 1}'
	...
	export default {
		...
		components: {
			'v-tree-item': {
				name: 'v-tree-item',
				template: 
				(`<ul class="cus_tree_ul" v-if="treeData && treeData.length" :class="{cus_tree_ulLine:(treeData && treeData.length)}">
					<li v-for="item in treeData">
						<div class="cus_item_content" @click="clickNode(item)" @click.cus="clickFun(JSON.stringify(item))" :id= item.id   :class="{active:item.active}"> 
							...
							<input type="text" v-model="item.text" @click="fn"/>
						</div>
						<v-tree-item :treeData="item.children"  v-if="item.expand" :clickFun="clickFun" :checkBox="checkBox" :class="{cus_checkbox_allchecked:item.checked}" @toggleCheckBox="checkBoxFun(item)" ></v-tree-item> 
					</li>
				</ul>`)
